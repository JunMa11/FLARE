...
if __name__ == "__main__":
    ...
    json_path = os.path.join(
        json_dir, glob.glob(args.docker_input_file + "/*.nii.gz")[0].split("/")[-1].split(".nii.gz")[0] + ".json",
    )
    ...
    try:
        ...
        # )
        cmd = 'docker container run --shm-size=28gb -m 28G --gpus="device=1" --name {} --rm -v $PWD/inputs/:/workspace/inputs/ -v $PWD/outputs/:/workspace/outputs/ {}:latest /bin/bash -c "sh predict.sh" '.format(args.docker_name, args.docker_name)
